...
if __name__ == '__main__':
    ...
    SOIL_SENSORS = 1
    ss_sensors = {f'ss_sensor_{hex(addr)}': STEMMA(addr=addr) for addr in range(0x36, 0x36 + SOIL_SENSORS)}

    while True:
        humidity = ss_sensors['ss_sensor_0x36'].get_moisture()

        payload = '{"mac_address": "00:00:00:00:00:00", ' \
                  '"temperature": "22", ' \
                  f'"humidity": "{humidity}", ' \
                  '"pressure": "888", ' \
                  '"adc": "4.1"}'
